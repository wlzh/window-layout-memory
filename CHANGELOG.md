...
- Telegram Lite单基准、单窗口时不依赖动态聊天标题恢复；有冲突时不自动新增铺满基准。
- 自动铺满保存没有手动跨屏证据时不改变已匹配基准的显示器。
- 保留其它应用匹配和手动保存行为；不自动猜测合并历史冲突记录。

暂无新增条目。

## [0.3.0-preview.19] - 2026-09-14

- 移除 Release 二进制中的本机源码绝对路径，并在签名前加入本机路径隐私门禁；功能、配置和权限不变。
- 微信聊天记录例外升级为按应用、标准窗口类型和聊天记录类别匹配，不再要求为每位联系人逐条添加。
- 旧聊天记录精确标题规则读取时合并为单一类别规则，不保留联系人姓名；其它窗口例外与布局保持不变。
- 连同preview.14至preview.18的竖屏留白、精确标题与文件类型例外、应用图标和打包门禁形成公开arm64预发布包。
- GitHub prerelease、SHA256和远端ZIP解压验证通过；仍为ad-hoc签名、未公证，真实多屏、辅助功能交互和长期性能门禁未全部完成。

## [0.3.0-preview.18] - 2026-09-12

- 补齐正式应用图标与CFBundleIconFile配置，Finder和关于窗口统一使用彩色图标，菜单栏继续单色显示。
- 新增可复现图标生成、10种表示验证、缺资源/错误引用负向测试和构建/安装门禁；完整测试脚本纳入文档检查。
- 不更改窗口布局与例外行为，无新增轮询、权限或外部依赖。

## [0.3.0-preview.17] - 2026-09-12

- 新增用户选择的文件扩展名永久例外，按应用、窗口类型及扩展名匹配，PDF等不绑定具体文件名；不读取内容。
- 移除微信图片硬编码，保留用户精确标题规则；所有例外优先于子窗口开关，横竖屏共用。
- 不自动扩大既有规则；本机微信PDF规则按用户授权一次性迁移。无新增轮询、权限或外部依赖。

## [0.3.0-preview.16] - 2026-09-12

- 新增菜单内自定义精确标题永久例外，按应用、类型和完整标题匹配，重启保留，横竖屏共用；无需AX标识或新增设置窗口。
- 保留原标识/临时规则，支持逐条撤销和上限保护；排除优先于子窗口选项，并阻止铺满模式下的恢复/学习旁路。

## [0.3.0-preview.15] - 2026-09-11

- 新增默认开启的“竖屏保留左侧留白”，关闭后竖屏左右贴可用工作区，保留纵向位置和高度。
- 持久保存选择；旧配置缺省开启，关闭时不学习或删除原留白，重新开启沿用原值。横屏及共享子窗口/例外规则不变，无新增轮询。

## [0.3.0-preview.14] - 2026-09-11

- 微信已知两个应用标识下的“图片和视频”标准窗口按子窗口处理，受现有子窗口铺满开关控制；聊天主窗口及其它应用保持原逻辑。仅匹配当前中文固定标题，无新增轮询或持久字段。

## [0.3.0-preview.13] - 2026-09-09

- 新增默认关闭、独立的竖屏横向撑满；沿用左侧留白，仅调整横向几何，纵向越界才修正。
- 复用应用/窗口例外、子窗口策略、手动换屏优先和成功结果保存；导入时两个方向自动撑满均关闭。
- 旧偏好缺少新字段时默认关闭，schema 1保持兼容，无新增后台轮询。

## [0.3.0-preview.12] - 2026-09-09

- 应用和窗口例外统一到“铺满例外”原生菜单列表，不弹管理窗口，区分永久应用、临时窗口和永久窗口规则。
- 保留已有持久规则、文件选择及未运行应用撤销；窗口目标从已核对前台记录取得。
- 显示空态和不可用原因，保留最多两级子菜单，不新增常驻轮询、刷新任务或管理窗口。
- 增强独立性能采样工具；旧build 17测量与当前功能验收分开记录。

## [0.3.0-preview.11] - 2026-09-09

- 自动铺满核验成功后更新当前组合下对应窗口的坐标、尺寸和显示器，保留历史。
- 明确鼠标换屏优先于旧保存归属，避免Finder类窗口反复回到旧屏幕。
- 失败、取消、锁定和歧义不覆盖；排队合并并复核，原值未变化不重复写盘。

## [0.3.0-preview.10] - 2026-09-09

- 按最新要求恢复先对齐左侧留白、再调整目标尺寸，不记录窗口成功历史。
- 失败停止并报告，不原生最大化、不自动回滚、不循环重试。受限窗口仍可能移动后未铺满。
- 无新增持久数据、依赖或定时器；普通恢复及所有例外保持不变。候选未通过实机前不发布。

## [0.3.0-preview.9] - 2026-09-09

- 撤回未通过实机的分步挪动实验，保留尺寸先核验、成功才写位置；新增事务级AXEnhancedUserInterface兼容处理并恢复原设置。
- 授权变化在下一次应用事件重新同步并核对，无新增后台轮询。
- 不使用原生最大化，不支持尺寸时不主动先搬到左侧；保留普通恢复及所有已有偏好，如实记录兼容性边界。

## [0.3.0-preview.8] - 2026-09-09

- 移除原生最大化回退，修复尺寸失败后可能留下全宽窗口、吞掉左留白的副作用路径。
- 保持100 pt及按组合保存的留白不变。尺寸受限制时停止，不先左移；尚不解决所有第三方应用尺寸兼容问题。

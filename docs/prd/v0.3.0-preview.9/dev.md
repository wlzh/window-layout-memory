...
新增LayoutCore.EdgeAnchoredPlacement，四边线性插值，每步不超过32 pt，最多128步。扩展起点边时小步位置/尺寸成对写入，收缩先尺寸，保持计划中的固定边。首分量和最终矩形分别最多4次读回，间隔30 ms；原点到位而尺寸未跟随时，最多重写本步尺寸3次，不继续移动原点，且要求几何仍在步骤范围及重试前未变化。适配器每次写入前检查8秒截止及原有权限、焦点、鼠标、组合、偏好保护。

...
**v0.3.0-preview.9 / build 15：开发预览，不是稳定版。** 新增原生[关于窗口](docs/ABOUT.md)，集中展示版本、作者、网站、GitHub、文档及MIT协议，适配明暗主题，关闭释放资源。包含[台前调度横屏自动铺满](docs/STAGE_FILL.md)：默认关闭，激活横屏前台窗口后避开菜单栏/Dock铺满，左侧默认100 pt且可通过左边缘拖动调整。按完整显示器组合和显示器ID记忆留白，不覆盖原布局。包含[只读布局预览](docs/PREVIEW.md)，关闭释放预览资源，不新增周期性扫描。尚未完整满足全部实机门禁。
...
build 11将菜单整理为12个顶层入口与四个功能组，保留常用操作；完整路径和作用域见[菜单导航](docs/MENU.md)。布局、100 pt默认值与持久偏好不变。本版[发行说明](docs/releases/v0.3.0-preview.9.md)。

build 15撤回未通过实机验收的分步挪动实验。保留先核验目标尺寸、成功才对齐位置的保护；新增临时AXEnhancedUserInterface兼容处理和授权事件恢复。尺寸不支持时不主动移动窗口到左侧、不使用原生最大化。候选实测与发布状态见[状态总览](docs/STATUS.md)。

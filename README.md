...
**v0.2.0-preview.2：开发预览，不是稳定版。** 修复鼠标松手后AX通知到达导致的漏记路径。包含[只读布局预览](docs/PREVIEW.md)：当前、保存、对比视图，屏幕按真实比例排列，窗口坐标/尺寸/差值可核对。关闭释放预览资源，不新增周期性扫描。尚未完整满足全部实机门禁。
...
详见 [使用指南](docs/USER_GUIDE.md)、[测试报告](docs/TESTING.md)、[兼容性](docs/COMPATIBILITY.md)、[发行说明](docs/releases/v0.2.0-preview.2.md)。

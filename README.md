...
**v0.3.0-preview.9 / build 14：开发预览，不是稳定版。** 新增原生[关于窗口](docs/ABOUT.md)，集中展示版本、作者、网站、GitHub、文档及MIT协议，适配明暗主题，关闭释放资源。包含[台前调度横屏自动铺满](docs/STAGE_FILL.md)：默认关闭，激活横屏前台窗口后避开菜单栏/Dock铺满，左侧默认100 pt且可通过左边缘拖动调整。按完整显示器组合和显示器ID记忆留白，不覆盖原布局。包含[只读布局预览](docs/PREVIEW.md)，关闭释放预览资源，不新增周期性扫描。尚未完整满足全部实机门禁。
...
build 14新增同屏边缘渐进调整：保留已对齐边，向未到位方向扩展，不先整窗搬到最终左侧，不使用原生最大化。同时修复授权后内部监控状态未恢复的问题，下一次应用事件自动重新核对。详见[铺满说明](docs/STAGE_FILL.md)。
